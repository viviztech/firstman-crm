...
vi.mock("@/jobs/lead-notifications", () => ({
  enqueueLeadAssignedNotification: vi.fn().mockResolvedValue(undefined),
}));
...
describe("POST /api/v1/leads (integration)", () => {
  afterAll(async () => {
    await db.delete(leads).where(ilike(leads.phone, "+919876601%"));
  });
  ...
  it("rejects invalid payloads with 400", async () => {
    const res = await POST(
      makeRequest({ phone: "not-a-phone" }, { token: env.LEADS_API_TOKEN, ip: randomUUID() }),
    );
    expect(res.status).toBe(400);
  });

  it("creates a lead with no staff creator and returns 201", async () => {
    const res = await POST(
      makeRequest(
        { name: "Public API Lead", phone: "9876601003", source: "website" },
        { token: env.LEADS_API_TOKEN, ip: randomUUID() },
      ),
    );
    ...
    const created = await db.query.leads.findFirst({ where: eq(leads.id, body.id) });
    expect(created?.name).toBe("Public API Lead");

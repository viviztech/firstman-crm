...
import { MAX_EMPLOYEE_IMPORT_BYTES } from "@/lib/hr-csv";
import { type HrActor, hasHrCapability } from "@/services/hr";

export async function authorizeImport(): Promise<HrActor | NextResponse> {
...
export async function readImportForm(
  request: Request,
): Promise<{ csvText: string; formData: FormData } | NextResponse> {
  ...
  if (file.size > MAX_EMPLOYEE_IMPORT_BYTES) {
    return NextResponse.json({ error: "Employee CSV exceeds the 1 MB limit" }, { status: 413 });
  }
  try {
    const csvText = new TextDecoder("utf-8", { fatal: true }).decode(await file.arrayBuffer());
    return { csvText, formData };
  } catch {

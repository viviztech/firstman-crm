...
export async function createClientAction(
  ...
  const created = await createClient(parsed.data, await toScope(currentUser));
...
export async function updateClientAction(
  ...
  const updated = await updateClient(id, parsed.data, await toScope(currentUser));
  if (!updated) {
...
export async function setWhatsAppOptOutAction(
  ...
  const updated = await setWhatsAppOptOut(id, optedOut, await toScope(currentUser));
  if (!updated) {
    return { ok: false, error: "Client not found, or you do not have access to it." };
  }
...
export async function deleteClientAction(id: string): Promise<ActionResult> {
  ...
  const deleted = await deleteClient(id, await toScope(currentUser));
  if (!deleted) {
